...
class Room(MappedAsDataclass, Base):
    ...
    number_of_beds: Mapped[Bed_type]= mapped_column(SAEnum(Bed_type, name= "bed_type"), nullable= False, init= True)
    possible_extra_beds: Mapped[Extra_beds]= mapped_column(SAEnum(Extra_beds, name= "extra_beds"), nullable= False, init= True)
    price_per_night: Mapped[float]= mapped_column(DECIMAL(10, 2), nullable= False, init= True)

    bookings: Mapped[list["Booking"]] = relationship(back_populates= 'room', init= False, uselist= True)
